...
import rmlmodels.C_AMCModel as camc
import csv
...
model = camc.Model(weights=None, input_shape=[512, 2], classes=11)
...
# Copy the weight parameters from the original model to the new model.
...
def predict(compress_model, cla_model):
    ...
    np.savetxt('C-AMC_accuracy.txt', acc_mod_snr_mean)
    plt.figure(figsize=(12, 10))
    plt.xlabel("Signal to Noise Ratio")
    plt.ylabel("Classification Accuracy")
