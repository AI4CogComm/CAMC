...
model = tf.keras.models.load_model(pruned_keras_file,custom_objects={'SumLayer': SumLayer, 'GaussianNoiseLayer':GaussianNoiseLayer
})
...
def predict(compress_model, cla_model):
    ...
    np.savetxt('SSCNet Pruned and Quantized accuracy.txt', acc_mod_snr_mean)
    plt.figure(figsize=(12, 10))
    plt.xlabel("Signal to Noise Ratio")
    plt.ylabel("Classification Accuracy")
