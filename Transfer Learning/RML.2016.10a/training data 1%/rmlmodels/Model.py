...
from keras.layers import LSTM,CuDNNLSTM,MultiHeadAttention,Flatten,LSTM,Reshape,Bidirectional
...
def Model(weights = None,
    ...
    x = Reshape((64, 1))(xc)
    x = Bidirectional(CuDNNLSTM(units=64, return_sequences=True), name='lstm3')(x)
    x = Dropout(dr)(x)
    x = Bidirectional(CuDNNLSTM(units=64), name='lstm4')(x)
    x = Reshape((1,128))(x)
...
if __name__ == '__main__':
    model = Model(None,input_shape=[128,2],classes=11)
