...
def upsample(signal):
    ...
    upsampled_signal = np.zeros((signal.shape[0], signal.shape[1] * 2))
...
def load_data(filename=r'D:\HECHAOWEI\data\Split-RML2016.10a-512\dataset2.pkl'):
    ...
    for mod in mods:
        for snr in snrs:
            ...
            for i in range(Xd[(mod, snr)].shape[0]):
                lbl.append((mod, snr))
            if (snr >= -12):
                train_idx += list(np.random.choice(range(a * 500, (a + 1) * 500), size=5, replace=False))
                val_idx += list(
                    np.random.choice(list(set(range(a * 500, (a + 1) * 500)) - set(train_idx)), size=5, replace=False))

                test_idx += list(set(range(a * 500, (a + 1) * 500)) - set(train_idx) - set(val_idx))
            else:
                test_idx += list(np.random.choice(range(a * 500, (a + 1) * 500), size=450, replace=False))
            a += 1
    X = np.vstack(X)
    X = np.array([downsample(x) for x in X])
    X = np.array([upsample(x) for x in X])
    print('X', X.shape)
