...
def apply_pruning_to_dense(layer):
    if isinstance(layer, PruneBidirectional):
        return prune.prune_low_magnitude(layer, pruning_schedule.PolynomialDecay(
            initial_sparsity=0.2, final_sparsity=0.8, begin_step=0, end_step=6600))
    if isinstance(layer, keras.layers.Dense):
        if layer.name == 'dense1':
            return prune.prune_low_magnitude(layer, pruning_schedule.PolynomialDecay(
                initial_sparsity=0.2, final_sparsity=0.7, begin_step=0, end_step=6600))
        else:
            return prune.prune_low_magnitude(layer, pruning_schedule.PolynomialDecay(
                initial_sparsity=0.2, final_sparsity=0.8, begin_step=0, end_step=6600))
...
def predict(compress_model, cla_model):
    ...
    np.savetxt('C-AMC-Pruned_accuracy.txt', acc_mod_snr_mean)
    plt.figure(figsize=(12, 10))
    plt.xlabel("Signal to Noise Ratio")
    plt.ylabel("Classification Accuracy")
